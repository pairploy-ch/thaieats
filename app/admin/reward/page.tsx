...
export default function AdminrewardsPage() {
  ...
  const applyAdd = async () => {
    ...
    const { data, error } = await supabase.from("reward").insert({ img: imageUrl }).select().single();
    if (!error && data) {
  ...
  const openDeleteModal = (reward: reward) => setDeleteModal({ reward, confirming: false });

  const applyDelete = async () => {
    if (!deleteModal) return;
    setDeleteModal((m) => m ? { ...m, confirming: true } : null);
    const { error } = await supabase.from("reward").delete().eq("id", deleteModal.reward.id);
    if (!error) {
      setrewards((prev) => prev.filter((p) => p.id !== deleteModal.reward.id));
      setDeleteModal(null);
  ...
  return (
    <div>
      ...
      <div className="toolbar">
        {!loading && <span className="badge-count">{rewards.length} rewards</span>}
        <button className="add-btn" onClick={openAddModal}>+ ADD reward</button>
      </div>
      ...
      {!loading && !error && (
        rewards.length === 0
          ? <div className="state-wrap">No rewards yet</div>
          : (
            <div className="grid">
              {rewards.map((promo, i) => (
                <div className="promo-card" key={promo.id}>
                  <div className="promo-img-wrap">
                    {promo.img
                      ? <Image src={promo.img} alt={`reward ${i + 1}`} fill style={{ objectFit: "contain" }} />
                      : <div style={{ width: "100%", height: "100%", display: "flex", alignItems: "center", justifyContent: "center", color: "#ddd", fontSize: 32 }}>�reward️</div>
                    }
                  </div>
      ...
      {deleteModal && (
        <div className="modal-overlay" onClick={() => !deleteModal.confirming && setDeleteModal(null)}>
          <div className="modal-box" onClick={(e) => e.stopPropagation()}>
            <h2 className="modal-title">Delete reward</h2>
         
            {deleteModal.reward.img && (
              <div style={{ position: "relative", width: "100%", aspectRatio: "1/1", marginBottom: 4, overflow: "hidden", border: "1px solid #f0f0f0" }}>
                <Image src={deleteModal.reward.img} alt="reward" fill style={{ objectFit: "cover" }} />
              </div>
            )}
            <div className="modal-actions">
